			...
			int status, const char *extra)
{
	...
	pr_debug("%02X %-25.25s %4i ?\t%s",ep_num, name, status, extra);
}
		...
		const u32 count, int status)
{
	...
	pr_debug("%02X %-25.25s %4i ?\t%d",ep_num, "DONE", status, count);
}
		...
		const struct usb_request *req, int status)
{
	...
	if (req != NULL) {
		...
		pr_debug("%02X %-25.25s %4i ?\t%d %d", ep_num, "QUEUE", status,
			!req->no_interrupt, req->length);
	}
}
		...
		const struct usb_ctrlrequest *req)
{
	...
	if (req != NULL) {
		...
		pr_debug("%02X %-25.25s ?\t%02X %02X %04X %04X %d",
			ep_num, "SETUP", req->bRequestType,
			req->bRequest, le16_to_cpu(req->wValue),
			le16_to_cpu(req->wIndex), le16_to_cpu(req->wLength));
	}
